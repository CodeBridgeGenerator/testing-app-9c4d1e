...
import nofityFakerFactory from "./nofityFakerFactory";
...
const NofityFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = nofityFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("nofity").create(elem));
...
export default connect(mapState, mapDispatch)(NofityFakerDialogComponent);
